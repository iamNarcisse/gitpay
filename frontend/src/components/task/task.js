...
class Task extends Component {
  ...
  async componentWillMount() {
    ...
    if (
      this.props.history &&
      this.props.history.location.pathname === `/task/${id}/orders`
    ) {
    ...
    if (
      this.props.history &&
      orderId &&
      this.props.history.location.pathname === `/task/${id}/orders/${orderId}`
    ) {
      this.props.changeTab(1);
    }
    if (
      this.props.history &&
      this.props.history.location.pathname === `/task/${id}/interested`
    ) {
      this.props.changeTab(2);
    }
    if (
      this.props.history &&
      this.props.history.location.pathname === `/task/${id}/members`
    ) {
      this.props.changeTab(3);
    }
    if (
      this.props.history &&
      this.props.history.location.pathname === `/task/${id}/offers`
    ) {
      this.props.changeTab(4);
    }
    if (
      this.props.history &&
      this.props.history.location.pathname === `/task/${id}/history`
    ) {
      this.props.changeTab(5);
    }
    if (
      this.props.history &&
      this.props.history.location.pathname === `/task/${id}/status`
    ) {
      if (logged) {
        if (
          this.props.task.data &&
          logged.user.id === this.props.task.data.userId
        ) {
        ...
        this.props.history.push({
          pathname: "/login",
          state: { from: { pathname: `/task/${id}/status` } },
        });
      }
    }
  }
  ...
  handleDeadlineForm = (e) => {
    e.preventDefault();
    this.state.deadlineForm
      ? this.setState({ deadlineForm: false })
      : this.setState({ deadlineForm: true });
  };
  ...
  handleInputInterestedCommentChange = (e) => {
    this.setState({
      interestedComment: e.target.value,
      charactersCount: e.target.value.length,
    });
  };
  ...
  handleCheckboxIwillDoFor = (e) =>
    this.setState({ priceConfirmed: e.target.checked });
  ...
  renderIssueAuthorLink = () => {
    if (
      this.props.task.data.metadata &&
      this.props.task.data.metadata.issue.user.html_url
    ) {
      return (
        <Link
        ...
        >
          <FormattedMessage
            ...
            values={{
              name: this.props.task.data.metadata
                ? this.props.task.data.metadata.issue.user.login
                : "unknown",
            }}
          />
        </Link>
  ...
  handleFundingEmailInputChange = (event) => {
    this.setState({
      fundingInvite: { ...this.state.fundingInvite, email: event.target.value },
    });
  };
  ...
  sendFundingInvite = (e) => {
    e.preventDefault();
    this.props.fundingInviteTask(
      this.props.task.data.id,
      this.state.fundingInvite.email,
      this.state.fundingInvite.comment,
      this.state.currentPrice,
      this.state.interestedSuggestedDate
    );
      ...
      <React.Fragment>
        <div>
          {this.props.task.values.available === 0
            ? this.props.intl.formatMessage(messages.taskValueLabelNoBounty)
            : `$ ${this.props.task.values.available}`}
        </div>
        {this.props.task.values.available === 0 && (
          <Button
        ...
        {!isOwner && (
          ...
          >
            <span className={this.props.classes.spaceRight}>
              <FormattedMessage
                id="this.props.ask.interested.offer"
  ...
  render() {
    ...
    const taskOwner = () => {
      const creator =
        this.props.logged && this.props.user.id === task.data.userId;
      const owner =
        task.data.members && task.data.members.length
          ? task.data.members.filter((m) => m.User.id === this.props.user.id)
              .length > 0
          : false;
    ...
    const isCurrentUserAssigned = () => {
      return (
        ...
        task.data.assignedUser.id === this.props.user.id
      );
    };
    ...
    const timePlaceholder = (
      ...
      >
        <FormattedMessage
          id="task.bounties.interested.created"
    ...
    const deliveryDate =
      task.data.deadline !== null
        ? MomentComponent(task.data.deadline).utc().format("DD-MM-YYYY")
        : this.props.intl.formatMessage(messages.deliveryDateNotInformed);
    const deadline =
      task.data.deadline !== null
            ...
            >
              {task.data.metadata ? (
                ...
                <div style={{ position: "absolute", left: 18, top: 5 }}>
                  <Typography color="default">
                    <FormattedMessage
                      id="task.status.author.missing"
                      defaultMessage="author info unknown"
                    />
                  </Typography>
                </div>
              )}
              ...
              <div className={classes.paper}>
                ...
                {!taskOwner() && (
                  ...
                  >
                    <span className={classes.spaceRight}>
                      <FormattedMessage
                        id="task.interested.action"
                        defaultMessage="Im interested"
                      />
                    </span>{" "}
                    <AddIcon />
                  </Button>
                ...
                {taskOwner() && (
                  <div style={{ display: "inline-block" }}>
                    ...
                    >
                      <span className={classes.spaceRight}>
                        <FormattedMessage
                          id="task.bounties.date.deadline"
                    ...
                    >
                      <span className={classes.spaceRight}>
                        <FormattedMessage
                          id="task.bounties.status.change"
                    ...
                    >
                      <span className={classes.spaceRight}>
                        <FormattedMessage
                          id="task.bounties.send.label"
                          defaultMessage="Send bounty"
                        />
                      </span>
                      <RedeemIcon />
                    </Button>
                    <StatusDialog
                      ...
                      providerStatus={
                        task.data.metadata
                          ? task.data.metadata.issue.state
                          : "unknown"
                      }
                ...
                <TaskAssignment
                  ...
                  handleInputInterestedCommentChange={
                    this.handleInputInterestedCommentChange
                  }
                  handleInputInterestedAmountChange={
                    this.handleInputInterestedAmountChange
                  }
                  ...
                  handleFundingEmailInputChange={
                    this.handleFundingEmailInputChange
                  }
                  handleFundingInputMessageChange={
                    this.handleFundingInputMessageChange
                  }
          ...
          <Grid container spacing={24}>
              ...
              {task.data.assigned && (
                <TaskAssigned
                  ...
                  status={this.props.intl.formatMessage(
                    Constants.STATUSES[task.data.status]
                  )}
                  classes={classes}
            ...
            <Grid item xs={12} sm={4}>
              ...
              <StatsCard
                ...
                statText={this.props.intl.formatMessage(
                  messages.taskValuesStatus,
                  {
              ...
              {MomentComponent(task.data.deadline).isValid() && (
                <StatsCard
                  ...
                  description={MomentComponent(task.data.deadline)
                    .utc()
                    .format("DD-MM-YYYY")}
                  statIcon={DateIcon}
